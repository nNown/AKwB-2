        ...
        for(auto& size : graphSizes) {
            for(auto& graph : graphTypes) {
                ...
                if(isAdjoint) {
                    ...
                    resultsFile << "linear: " << (isLinear ? "true" : "false") << "\t" << "Transformed graph" << "\t" << "size: " << transformedGraph.Vertices().size() << "\t"<< "multi: " << (isTransformedSimple ? "true" : "false") << "\t" << "adjoint:" << (isTransformedAdjoint ? "true" : "false") << "\t" << "linear:" << (isTransformedLinear ? "true" : "false") << std::endl;
                    continue;
                }
